...
import { createBankCheckRequest } from '../../Redux/Actions/BankCheckInfoAction'
...
const NewCheckPart = () => {
  ...
  const handleSubmit = event => {
    ...
    if (validate() && unp !== '') {
      ...
      dispatch(createBankCheckRequest(check))
    }
  }
        ...
        <Grid container spacing={2}>
          ...
          <Grid className='justify-content-xs' item xs={12}>
            <FormControlLabel
              control={
                <Checkbox
                  checked={values.isDebtRepayment}
                  onChange={handleChange}
                  name='isDebtRepayment'
                />
              }
              className={classes.formControl}
